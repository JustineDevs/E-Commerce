...
/**
 * Removes `<app>/.next` before `next build`. On Windows, a single rm can fail with
 * ENOTEMPTY/EPERM when handles are still releasing (dev server, AV, parallel turbo).
 * Retries with backoff before failing.
 */
...
async function removeNextDir() {
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      ...
      await new Promise((r) =>
        setTimeout(r, BASE_DELAY_MS * (attempt + 1)),
      );
    }
  }
