...
import { listCmsCategoryContent, upsertCmsCategoryContent } from "@apparel-commerce/platform-data";
...
export async function GET(req: NextRequest) {
  ...
  const data = await listCmsCategoryContent(sup.client);
...
export async function POST(req: NextRequest) {
  ...
  const sup = adminSupabaseOr503(cid);
  if ("response" in sup) return sup.response;
  const data = await upsertCmsCategoryContent(sup.client, body as Parameters<typeof upsertCmsCategoryContent>[1]);
