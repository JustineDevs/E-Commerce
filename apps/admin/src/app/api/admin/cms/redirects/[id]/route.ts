...
import { deleteCmsRedirect, upsertCmsRedirect } from "@apparel-commerce/platform-data";
...
export async function PUT(req: NextRequest, ctx: RouteCtx) {
  ...
  const data = await upsertCmsRedirect(sup.client, {
    ...(body as object),
    id,
  } as Parameters<typeof upsertCmsRedirect>[1]);
...
export async function DELETE(req: NextRequest, ctx: RouteCtx) {
  ...
  const sup = adminSupabaseOr503(cid);
  if ("response" in sup) return sup.response;
  const ok = await deleteCmsRedirect(sup.client, id);
