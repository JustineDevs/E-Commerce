 ...
 * When DATABASE_URL is missing, returns false (fail-closed) to prevent duplicate processing.
 */
export async function claimLemonWebhookDedup(dedupId: string): Promise<boolean> {
  ...
  const p = getPool();
  if (!p) {
    console.warn("[lemon-dedup] DATABASE_URL not set — rejecting webhook to prevent duplicate processing");
    return false;
  }
  await ensureTable(p);
