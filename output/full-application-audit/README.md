...
**Last reconciled:** 2026-03-31 (examples: in-app catalog matrix and chat-order intake evolved, audit log explorer uses plain-language labels). Payment provider credentials are **Medusa `process.env` only**; Supabase BYOK paths described in some registers below are **historical**.

**Security (live):** See `docs/security-program.md` for Medusa env expectations, admin API guard script, and CI wiring.
